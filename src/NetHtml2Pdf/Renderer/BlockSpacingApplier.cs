...
internal sealed class BlockSpacingApplier : IBlockSpacingApplier
{
    public IContainer ApplySpacing(IContainer container, CssStyleMap styles)
    {
        ...
        {
            if (styles.Padding.Top.HasValue) container = container.PaddingTop((float)styles.Padding.Top.Value);

            if (styles.Padding.Right.HasValue) container = container.PaddingRight((float)styles.Padding.Right.Value);

            if (styles.Padding.Bottom.HasValue) container = container.PaddingBottom((float)styles.Padding.Bottom.Value);

            if (styles.Padding.Left.HasValue) container = container.PaddingLeft((float)styles.Padding.Left.Value);
        }
    ...
    public IContainer ApplyMargin(IContainer container, CssStyleMap styles)
    {
        if (styles.Margin.HasValue)
        {
            ...
            if (styles.Margin.Top.HasValue) container = container.PaddingTop((float)styles.Margin.Top.Value);

            if (styles.Margin.Right.HasValue) container = container.PaddingRight((float)styles.Margin.Right.Value);

            if (styles.Margin.Bottom.HasValue) container = container.PaddingBottom((float)styles.Margin.Bottom.Value);

            if (styles.Margin.Left.HasValue) container = container.PaddingLeft((float)styles.Margin.Left.Value);
        }
