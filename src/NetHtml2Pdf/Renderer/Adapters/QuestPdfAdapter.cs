...
internal sealed class QuestPdfAdapter : IRendererAdapter
{
    ...
    public void Render(PageFragmentTree page, RendererContext context)
    {
        ...
        if (!_documentBegun) throw new InvalidOperationException("BeginDocument must be called before Render.");
    ...
    {
        if (!_documentBegun) throw new InvalidOperationException("BeginDocument must be called before EndDocument.");
    ...
    {
        if (_context is null) throw new InvalidOperationException("Renderer context is not available.");
                ...
                container.Page(pageDescriptor =>
                {
                    ...
                    if (_context.Header is not null)
                        pageDescriptor.Header()
                            .Element(header => { RenderFragment(header, _context.Header, _context); });

                    if (_context.Footer is not null)
                        pageDescriptor.Footer()
                            .Element(footer => { RenderFragment(footer, _context.Footer, _context); });

                    pageDescriptor.Content().Column(column =>
                    {
                        foreach (var slice in page.Fragments)
                            column.Item().Element(item => { RenderFragment(item, slice.SourceFragment, _context); });
                    });
                });
        });
    ...
    {
        if (node.NodeType == DocumentNodeType.Text) return node.TextContent ?? string.Empty;

        if (node.Children.Count == 0) return string.Empty;

        var builder = new StringBuilder();
        foreach (var child in node.Children) builder.Append(ExtractText(child));
