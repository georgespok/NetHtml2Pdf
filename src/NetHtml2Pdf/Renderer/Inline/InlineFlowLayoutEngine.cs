    ...
    private static void ValidateNestedBlockSpacing(DocumentNode blockChild, DocumentNode inlineBlockParent)
    {
        ...
        if (blockMargin.HasValue)
        {
            if (blockMargin.Top.HasValue && blockMargin.Top.Value < 0)
                throw new InvalidOperationException($"Negative top margin ({blockMargin.Top.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");

            if (blockMargin.Right.HasValue && blockMargin.Right.Value < 0)
                throw new InvalidOperationException($"Negative right margin ({blockMargin.Right.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");

            if (blockMargin.Bottom.HasValue && blockMargin.Bottom.Value < 0)
                throw new InvalidOperationException($"Negative bottom margin ({blockMargin.Bottom.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");

            if (blockMargin.Left.HasValue && blockMargin.Left.Value < 0)
                throw new InvalidOperationException($"Negative left margin ({blockMargin.Left.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");
        }
        ...
        if (blockPadding.HasValue)
        {
            if (blockPadding.Top.HasValue && blockPadding.Top.Value < 0)
                throw new InvalidOperationException($"Negative top padding ({blockPadding.Top.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");

            if (blockPadding.Right.HasValue && blockPadding.Right.Value < 0)
                throw new InvalidOperationException($"Negative right padding ({blockPadding.Right.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");

            if (blockPadding.Bottom.HasValue && blockPadding.Bottom.Value < 0)
                throw new InvalidOperationException($"Negative bottom padding ({blockPadding.Bottom.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");

            if (blockPadding.Left.HasValue && blockPadding.Left.Value < 0)
                throw new InvalidOperationException($"Negative left padding ({blockPadding.Left.Value}) in nested block element {blockChild.NodeType} within inline-block {inlineBlockParent.NodeType}");
        }
    }
