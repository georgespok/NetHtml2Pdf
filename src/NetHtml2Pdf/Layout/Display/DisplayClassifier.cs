...
/// </summary>
internal sealed class DisplayClassifier : IDisplayClassifier
{
    ...
    {
        _logger = logger;
        _enableTraceLogging = options?.EnableClassifierTraceLogging ?? false;
    }
    ...
    public DisplayClass Classify(DocumentNode node, CssStyleMap style)
    {
        ...
        if (style.DisplaySet && style.Display != CssDisplay.Default)
        {
            ...
            {
                _logger?.LogDebug("DisplayClassifier: Node {NodeType} classified as {DisplayClass} via CSS display {CssDisplay}", 
        ...
        else
        {
            ...
            if (_enableTraceLogging)
            {
                _logger?.LogDebug("DisplayClassifier: Node {NodeType} classified as {DisplayClass} via semantic default", 
    ...
    private DisplayClass HandleUnsupportedDisplayValue(CssDisplay cssDisplay, DocumentNodeType nodeType)
    {
        ...
        {
            _logger?.LogWarning("Unsupported CSS display value '{DisplayValue}' for node type '{NodeType}'. Falling back to semantic default.", 
