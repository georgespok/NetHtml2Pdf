...
public partial class PdfRenderSnapshot(
    ...
    long memoryUsage)
{
    ...
    public int ElementCount { get; } =
        ValidateElementCount(elementCount, supportedElementCount, fallbackElementCount, fallbackElements);
    ...
    public void SetValidation(DateTime validationTimestamp, string validationResult)
    {
        ...
        if (validationResult != null && !ValidValidationResults.Contains(validationResult))
            throw new ArgumentException(
                $"ValidationResult must be one of '{string.Join("', '", ValidValidationResults)}', or null",
    ...
    private static string ValidatePlatform(string platform)
    {
        ...
        if (!ValidPlatforms.Contains(platform))
            throw new ArgumentException($"Platform must be exactly '{string.Join("' or '", ValidPlatforms)}'",
                nameof(platform));
    ...
    {
        if (warnings == null) throw new ArgumentException("Warnings cannot be null", nameof(warnings));
    ...
    {
        if (outputSize <= 0) throw new ArgumentException("OutputSize must be positive", nameof(outputSize));

        const long minSize = 1024; // 1KB
        const long maxSize = 100L * 1024 * 1024; // 100MB

        if (outputSize < minSize) throw new ArgumentException("OutputSize must be at least 1KB", nameof(outputSize));

        if (outputSize > maxSize) throw new ArgumentException("OutputSize must not exceed 100MB", nameof(outputSize));
    ...
    private static DateTime ValidateTimestamp(DateTime timestamp)
    {
        ...
        if (timestamp > now) throw new ArgumentException("Timestamp cannot be in the future", nameof(timestamp));

        var minDate = new DateTime(2020, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        if (timestamp < minDate) throw new ArgumentException("Timestamp cannot be before 2020", nameof(timestamp));
    ...
    private static List<string> ValidateFallbackElements(List<string> fallbackElements)
    {
        ...
        foreach (var element in fallbackElements)
        {
            ...
            if (!HtmlTagNameRegex.IsMatch(element))
                throw new ArgumentException("FallbackElements must be valid HTML tag names (letters, numbers, hyphens)",
                    nameof(fallbackElements));
        }
    ...
    {
        if (inputHtmlSize < 0) throw new ArgumentException("InputHtmlSize must be non-negative", nameof(inputHtmlSize));

        return inputHtmlSize;
    }

    private static int ValidateElementCount(int elementCount, int supportedElementCount, int fallbackElementCount,
        List<string> fallbackElements)
    {
        if (elementCount < 0) throw new ArgumentException("ElementCount must be non-negative", nameof(elementCount));

        if (fallbackElementCount != fallbackElements.Count)
            throw new ArgumentException("FallbackElementCount must equal FallbackElements.Count",
                nameof(fallbackElementCount));

        if (elementCount != supportedElementCount + fallbackElementCount)
            throw new ArgumentException("ElementCount must equal SupportedElementCount + FallbackElementCount",
                nameof(elementCount));
    ...
    private static int ValidateSupportedElementCount(int supportedElementCount, int elementCount)
    {
        ...
        if (supportedElementCount > elementCount)
            throw new ArgumentException("SupportedElementCount cannot exceed ElementCount",
                nameof(supportedElementCount));
    ...
    {
        if (memoryUsage < 0) throw new ArgumentException("MemoryUsage must be non-negative", nameof(memoryUsage));
