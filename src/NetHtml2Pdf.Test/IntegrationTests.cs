...
public class IntegrationTests : PdfRenderTestBase
{
    ...
    public async Task PdfBuilder_DisplayInline_RendersInline()
    {
        ...
        // Assert
        AssertValidPdf(pdfBytes);
        await this.SavePdfForInspectionAsync(pdfBytes);
    ...
    public async Task FullDocument_Rendering_SmokeTest()
    {
        ...
        {
            Output.WriteLine($"Could not find inline-block words. Left found: {leftWord != null}, Right found: {rightWord != null}");
        }
        ...
        if (leftRawWord != null && rightRawWord != null)
        {
            ...
            Output.WriteLine($"Left word position: ({leftRawWord.BoundingBox.TopLeft.X:F1}, {leftRawWord.BoundingBox.TopLeft.Y:F1})");
            Output.WriteLine($"Right word position: ({rightRawWord.BoundingBox.TopLeft.X:F1}, {rightRawWord.BoundingBox.TopLeft.Y:F1})");
    ...
    public async Task InlineBlock_MixedContent_ShouldFlowCorrectly()
    {
        ...
        if (wordC != null) Output.WriteLine($"  C: ({wordC.BoundingBox.TopLeft.X:F1}, {wordC.BoundingBox.TopLeft.Y:F1})");
        if (wordD != null) Output.WriteLine($"  D: ({wordD.BoundingBox.TopLeft.X:F1}, {wordD.BoundingBox.TopLeft.Y:F1})");
