...
namespace NetHtml2Pdf.Parser.Utilities
{
    /// <summary>
    ...
    public class StyleParser : IStyleParser
    {
        public void ApplyInlineStyles(IElement element, DocumentNode node)
        {
            ...
            {
                node.Style.Alignment = textAlign.ToLowerInvariant() switch
                {
            ...
            if (styles.TryGetValue("margin", out var margin))
            {
                ...
                {
                    node.Style.Box.MarginTop = marginValue.Value;
                    node.Style.Box.MarginRight = marginValue.Value;
                    node.Style.Box.MarginBottom = marginValue.Value;
                    node.Style.Box.MarginLeft = marginValue.Value;
                }
            }
            ...
            if (styles.TryGetValue("padding", out var padding))
            {
                ...
                {
                    node.Style.Box.PaddingLeft = paddingValue.Value;
                    node.Style.Box.PaddingRight = paddingValue.Value;
                    node.Style.Box.PaddingTop = paddingValue.Value;
                    node.Style.Box.PaddingBottom = paddingValue.Value;
                }
            }
            ...
            if (styles.TryGetValue("padding-left", out var paddingLeft))
            {
                ...
                {
                    node.Style.Box.PaddingLeft = paddingLeftValue.Value;
                }
            }
            ...
            if (styles.TryGetValue("padding-right", out var paddingRight))
            {
                ...
                {
                    node.Style.Box.PaddingRight = paddingRightValue.Value;
                }
            }
            ...
            if (styles.TryGetValue("padding-top", out var paddingTop))
            {
                ...
                {
                    node.Style.Box.PaddingTop = paddingTopValue.Value;
                }
            }
            ...
            if (styles.TryGetValue("padding-bottom", out var paddingBottom))
            {
                ...
                {
                    node.Style.Box.PaddingBottom = paddingBottomValue.Value;
                }
            }
        ...
        public void ApplyInlineStyles(IElement element, TableCellNode cell)
        {
            ...
            if (styles.TryGetValue("text-align", out var textAlign))
            {
                cell.Style.Alignment = textAlign.ToLowerInvariant() switch
                {
                    "center" => TextAlignment.Center,
                    "right" => TextAlignment.Right,
                    "justify" => TextAlignment.Justify,
            ...
            if (styles.TryGetValue("padding", out var padding))
            {
                ...
                if (value.HasValue)
                {
                    cell.Style.Box.PaddingLeft = value.Value;
                    cell.Style.Box.PaddingRight = value.Value;
                    cell.Style.Box.PaddingTop = value.Value;
                    cell.Style.Box.PaddingBottom = value.Value;
                }
            }
            if (styles.TryGetValue("padding-left", out var cpl)) cell.Style.Box.PaddingLeft = ParseSize(cpl) ?? cell.Style.Box.PaddingLeft;
            if (styles.TryGetValue("padding-right", out var cpr)) cell.Style.Box.PaddingRight = ParseSize(cpr) ?? cell.Style.Box.PaddingRight;
            if (styles.TryGetValue("padding-top", out var cpt)) cell.Style.Box.PaddingTop = ParseSize(cpt) ?? cell.Style.Box.PaddingTop;
            if (styles.TryGetValue("padding-bottom", out var cpb)) cell.Style.Box.PaddingBottom = ParseSize(cpb) ?? cell.Style.Box.PaddingBottom;

            // Border
            ParseBorder(styles, out var bw, out var bc);
            if (bw.HasValue) cell.Style.Border.Left.Width = bw.Value;
            if (!string.IsNullOrEmpty(bc)) cell.Style.Border.Left.ColorHex = ConvertColorToHex(bc);
        }
